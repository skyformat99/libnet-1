...
int tcprecvuntil(int sock, char *buf, size_t len, const char *s,
                 size_t s_len) {
	size_t i;
	for (i = 0; i != len; ++i, ++buf) {
...
int udpsend(int sock, ip ip, const char *buf, size_t len) {
	if (ip.iptype == IPV4) {
		return sendto(sock, buf, len, 0, (struct sockaddr *) &ip.ipaddress.ipv4,
		              sizeof(ip.ipaddress.ipv4));
	} else {
		return sendto(sock, buf, len, 0, (struct sockaddr *) &ip.ipaddress.ipv6,
		              sizeof(ip.ipaddress.ipv6));
	}
}
...
int udprecv(int sock, ip ip, char *buf, size_t len) {
	if (ip.iptype == IPV4) {
		int fromlen = sizeof(ip.ipaddress.ipv4);
		return recvfrom(sock, buf, len, 0, (struct sockaddr *) &ip.ipaddress.ipv4,
		                &fromlen);
	} else {
		int fromlen = sizeof(ip.ipaddress.ipv6);
